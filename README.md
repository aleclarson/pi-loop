...
- **Backend & Webhooks (`@goddard-ai/backend`)**: Edge services that intercept GitHub events and instantly stream them to your local daemon via Server-Sent Events (SSE), enabling instant AI responses to PR feedback.
...
Use `@goddard-ai/backend` to forward webhook events to backend:

```bash
pnpm --dir=core/backend exec tsx --eval "import { createGitHubApp } from './src/index.ts'; (async () => { const app = createGitHubApp({ backendBaseUrl: 'http://127.0.0.1:8787' }); await app.handleWebhook({ type: 'issue_comment', owner: 'owner', repo: 'repo', prNumber: 1, author: 'teammate', body: 'Looks good' }); console.log('webhook sent'); })();"
```
